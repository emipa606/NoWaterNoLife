...
namespace MizuMod
{
    public static class Toils_Mizu
    {
            ...
            toil.initAction = delegate
                {
                    ...
                    if (!(thing is IBuilding_DrinkWater building) || building.WaterType == WaterType.Undefined || building.WaterType == WaterType.NoWater || !building.CanDrawFor(actor))
                    {
        ...
        public static Toil DrinkFromBuilding(TargetIndex buildingIndex)
        {
            ...
            toil.initAction = delegate
                {
                    ...
                    if (actor.needs?.Water() == null || !(thing is IBuilding_DrinkWater) || comp == null || !comp.IsWaterSource)
                    {
                        actor.jobs.EndCurrentJob(JobCondition.Incompletable);
                        return;
                    }
            ...
            toil.WithProgressBar(buildingIndex, () => 1f - ((float)toil.actor.jobs.curDriver.ticksLeftThisToil / initialTicks));
            toil.defaultCompleteMode = ToilCompleteMode.Delay;
            toil.FailOn(
                _ =>
                    {
        ...
        public static Toil DrinkTerrain(TargetIndex cellIndex, int baseDrinkTicksFromTerrain)
        {
            ...
            toil.WithProgressBar(cellIndex, () => 1f - ((float)toil.actor.jobs.curDriver.ticksLeftThisToil / initialTicks));
            toil.defaultCompleteMode = ToilCompleteMode.Delay;
            toil.FailOn(
                _ =>
                    {
            ...
            toil.initAction = delegate
                {
                    ...
                    // その場に置いてみる
                    var isDropSuccess = actor.carryTracker.TryDropCarriedThing(actor.CurJob.GetTarget(dropSpotIndex).Cell, ThingPlaceMode.Direct, out var dropThing);
        ...
        public static T FailOnChangingTerrain<T>(this T f, TargetIndex index, List<WaterTerrainType> waterTerrainTypeList)
            where T : IJobEndable
        {
            f.AddEndCondition(
                () =>
            ...
            toil.initAction = delegate
                {
                    ...
                    {
                        // 直接飲んだ
                        actor.needs.mood.thoughts.memories.TryGainMemory(actor.CanManipulate() ? MizuDef.Thought_DrankScoopedWater : MizuDef.Thought_SippedWaterLikeBeast);
            ...
            toil.initAction = delegate
                {
                    ...
                    if (StoreUtility.TryFindBestBetterStoreCellFor(thing, actor, actor.Map, StoragePriority.Unstored, actor.Faction, out var c))
                    {
            ...
            toil.initAction = () =>
                {
                    ...
                    if (!StoreUtility.TryFindBestBetterStoreCellFor(thing, actor, actor.Map, StoragePriority.Unstored, actor.Faction, out var storeCell))
                    {
